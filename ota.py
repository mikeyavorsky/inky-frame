# ota.py - minimal pull-based OTA updater for Inky Frame (MicroPython)
#
...
#
# The Inky Frame deep-sleeps between refreshes, so each wake is effectively a
# fresh boot - this module is called once per wake from main.py, no long-running
# poll loop needed.
...
def _parse_version(v):
    ...
    # compare would put "10" before "9"). E.g. "2026-06-29-10" -> [2026,6,29,10].
    parts, num = [], ""
    for ch in v:
        if ch.isdigit():
            num += ch
